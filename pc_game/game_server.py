...
# ビート0が来るまでのカウントイン拍数。画面側はこの拍に合わせて「3・2・1」を大きく出し、
# そのままノーツに入る。以前は4拍で、さらにステージクリア側でも3・2・1を出していたため
# 「3,2,1,4...」と二重に数えているように見えていた。カウントダウンはここ1本に統一する。
COUNT_IN_BEATS = 3
# ステージクリア後、ONを押させずに自動で次のステージへ進むまでの時間。
# ここでは「STAGE N CLEAR!」と次ステージの予告だけを見せる(数字は出さない)。
STAGE_CLEAR_COUNTDOWN_SEC = 2.0
