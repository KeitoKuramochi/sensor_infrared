...
**自動反映されるファイル名(拡張子は png / jpg / jpeg / webp どれでも可):**
`bg` `logo` `gameover` `allclear` `stageclear`(ステージクリアのスタンプ)
`excellent`(EXCELLENT判定の瞬間のバースト) `mascot`(遊び方パネル・透過PNG推奨)
`heart`(ライフ表示・透過PNG推奨)

> 2026-07-27: ユーザー生成の8枚を加工(トリミング・透過処理)して配置済み。
> 差し替えたいときは同じファイル名で上書きすればOK。

## ゲームに自動反映される画像(ファイル名厳守)
